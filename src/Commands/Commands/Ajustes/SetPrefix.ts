...
export const command: Command = {
    ...
    run: async (Komi, message, args) => {
        ...
        let UserPerms = message.member.permissions.has("ADMINISTRATOR") || message.member.permissions.has("MANAGE_GUILD")
        ...
        if (!Prefix) {
            message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription(`${Util.No} | Debes escribir un nuevo **__Prefijo__** de máximo 4 caracteres.`)
                        .setColor("#990000")
                ]
            })
            return;
            ...
            message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription(`${Util.No} | No puedes usar más de 4 caracteres para el nuevo **__Prefijo__**.`)
                        .setColor("#990000")
                ]
            })
            return;
        ...
        message.reply({
            embeds: [
                new MessageEmbed()
                    .setDescription(`${Util.Yes} | El Prefijo fue cambiado a **__${Prefix}__**`)
                    .setColor("#009900")
            ]
        })
    }
