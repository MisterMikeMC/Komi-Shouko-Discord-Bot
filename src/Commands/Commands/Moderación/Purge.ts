    ...
    run: async (Komi, message, args) => {
        ...
        if (!Arguments) {
            message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription(`${Util.No} | Necesitas proporcionar una cantidad de mensajes para borrar.`)
                        .setColor("#990000")
                ]
            })
            return;
            ...
            message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription(`${Util.No} | Necesitas ingresar una cantidad valida.`)
                        .setColor("#990000")
                ]
            })
            return;
        ...
        ChannelForBulk.bulkDelete(DeleteMessageNumber + 1)
            .then(bulk => {
                message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setDescription(`${Util.Yes} | Se han borrado **${DeleteMessageNumber}** correctamente.`)
                            .setColor("#009900")
                    ]
                }).then(msg => {
                    setTimeout(() => {
            ...
            .catch(err => {
                message.reply({
                    embeds: [
                        new MessageEmbed()
                        .setDescription(`${Util.No} | Hubo un error al intentar borrar los mensajes.`)
                        .setColor("#990000")
                    ]
                })
                return;
