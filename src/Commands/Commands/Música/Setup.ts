...
export const command: Command = {
  ...
  run: async (Komi, message, args): Promise<void> => {
    ...
    try {
      if (
        ...
        message.guild.channels.cache.find(
          (channel) => channel.id === `${MusicChannel}`
        )
      ) {
        message.reply({
          embeds: [
    ...
    const ButtonLine1 = new MessageActionRow().addComponents(
      new MessageButton()
    ...
    const ButtonLine2 = new MessageActionRow().addComponents(
      new MessageButton()
    ...
    message.guild.channels
      ...
      .then(async (musicChannel) => {
        ...
        musicChannel
          .send({
            content: `**Komi Queue:**\n\n${QueueStatus}`,
            embeds: [MusicTableEmbed],
            components: [ButtonLine1, ButtonLine2],
          })
          .then(async (musicMessage) => {
            ...
            message.reply({
              embeds: [
                new MessageEmbed()
                  .setDescription(
                    `${Util.Yes} | Canal de música creado: <#${musicChannel.id}>`
                  )
                  .setColor("#009900"),
              ],
              allowedMentions: {
