...
export const command:Command = {
    ...
    run: async(Komi, message, args) => {
        let User = message.mentions.members.first() || Komi.users.resolve(args[0]) || Komi.users.cache.get(args[0])
        if(!User){
            message.reply({
                embeds: [
                    new MessageEmbed()
                        .setDescription(`${Util.No} | Necesitas mencionar a un usuario o colocar su id.`)
                        .setColor("#990000")
                ]
            })
        }
