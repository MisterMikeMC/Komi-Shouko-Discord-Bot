...
import { Ping, Util, KomiShouko, Badge } from '../../../File Data/Util/Emojis.json'
export default new SlashCommandStructure({
    ...
    run: async ({ Komi, interaction }) => {
        if (interaction.options.getSubcommand() === 'ping') {
            ...
            interaction.reply({
                embeds: [
                    new MessageEmbed()
                        ...
                        .addFields(
                            { name: `${Util.WindL} Ping de Respuesta: ${Util.WindL}`, value: `> ¡**__${PingRespuesta}__** ms! ${PingEmojiFinal1}`, inline: false },
                            { name: `${Util.WindL} Ping de la API: ${Util.WindL}`, value: `> ¡**__${PingApi}__** ms! ${PingEmojiFinal2}`, inline: false },
                            { name: `${Util.WindL} Ram usada: ${Util.WindL}`, value: `> **__${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}__** MB`, inline: false }
                        )
                        .setFooter({
            ...
            const displayRoleColor = interaction.guild.me.displayHexColor === "#000000" ? "#ffffff" : interaction.guild.me.displayHexColor
            interaction.reply({
                embeds: [
                    new MessageEmbed()
                        .setAuthor({
                        ...
                        .setDescription(`${KomiShouko.KomiOK} Aquí tienes la invitación para que me invites a tu servidor. ${KomiShouko.KomiOK}`)
                        .setImage("https://media.discordapp.net/attachments/880562658401722379/934945810301468782/komi-cant-communicate-komi.gif")
                ...
                components: [
                    new MessageActionRow()
                        .addComponents(
                            new MessageButton()
                                .setStyle("LINK")
                                .setURL("https://discord.com/api/oauth2/authorize?client_id=875166925884370994&permissions=8&scope=applications.commands%20bot")
                                .setLabel("Invitame"),
                            new MessageButton()
                                .setStyle("LINK")
            ...
            interaction.reply({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setImage("https://cdn.discordapp.com/attachments/930674284425265182/934612890219085884/standard.gif")
                        .setColor(`${roleColor}`)
                        .setFooter({
                            text: `Comando hecho por ${interaction.user.tag}`,
