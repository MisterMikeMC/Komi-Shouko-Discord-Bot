        ...
        if (interaction.options.getSubcommand() === 'embeds') {
            ...
            if (Type === 'verificacion') {
                interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setDescription(`${Util.Yes} | Se ha mandado el embed correctamente.`)
                            .setColor("#00FF00")
                    ],
                    ephemeral: true
                })
                ...
                interaction.reply({
                    embeds: [
                        new MessageEmbed().setDescription(`${Util.Yes} | Se ha mandado el embed correctamente.`).setColor("#00FF00")
                    ],
                    ephemeral: true
                })
                interaction.channel.send({
                    embeds: [
                        new MessageEmbed().setAuthor({ name: "¡Bienvenido(a) al soporte de Komi Shouko!", iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}` }).setDescription(`¡Hola **${interaction.user.tag}**, bienvenido(a) al soporte de **__🌸 Komi Shouko 🌸__** esperamos que sea de tu agrado este servidor.`).setImage("https://cdn.discordapp.com/attachments/880562658401722379/934945897295527956/komi-san-komi-shouko_1.gif").setColor("#FFCC8E").setThumbnail(interaction.user.displayAvatarURL({ dynamic: true })).setFooter({ text: `¡Wow contigo ya somos ${interaction.guild.memberCount} Miembros!`, iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}` }).setTimestamp()
                    ]
                })
                ...
                interaction.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setAuthor({ name: "Bye bye~", iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}` }).setDescription(`Lamentablemente **${interaction.user.tag}** decidio irse del servidor, hasta luego.`).setImage("https://cdn.discordapp.com/attachments/880562658401722379/934945897954017310/komi-san-komi-shouko_2.gif").setColor("#FFCC8E").setThumbnail(interaction.user.displayAvatarURL({ dynamic: true })).setFooter({ text: `Ahora solo somos ${interaction.guild.memberCount} Miembros.`, iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}` }).setTimestamp()
                    ]
                })
            }
